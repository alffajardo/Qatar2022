...
picks_id <- drive_find(type = "spreadsheet",pattern = "KO_Stage2_respuestas",
n_max = 1)$id
...
write.table(bets,"KO2_picks.csv",sep=",",
quote = F,
row.names =F )


write.table(scores_prediction,"KO2_predicted_scores.csv",sep=",",
            quote = F,
            row.names =F )
...
ggsave("picks_KO2.png",plot = plot1,units = "px",width = 1200,height = 1200,
...
## calculate an histogram
 ...
 plot2 <- scores_prediction %>%
  ...
  ggsave("predicted_scores_KO2.png",plot = plot2,units = "px",width = 1200,height = 1000,
        path = "media",dpi = 200,bg="white")
